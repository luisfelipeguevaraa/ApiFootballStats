...
def average_stats_df(games_slide, team_list, team_fixture_id_dict, game_stats, making_predictions=False):
    '''
    Output is a dataframe of averaged game stats.
    Included is a teams average stats over 'games_slide' number of
    games as well as the avergae opponent stats in those games.
...
def mod_df(df, making_predictions=False):
    ...
    It takes a team and their oppoents (in the last 10 games) average stats,
    and subtracts one from the other. The benefit of this is it provides a more useful metric
    for how well a team has been performing. If the 'Av Shots Diff' is positive,
    it means that team has, on average taken more shots than their opponent in the previous games.
    ...
    # in our input dataframe (df) we have removed data from teams that have played less than 5 or 10 games.
    # However, we havent removed data from the oposing team which has played more than 5 or 10 games.
    # In the input df we have two rows for each fixture, one for each teams stats. The code below removes
    # the data of all games that only have a single teams stats available, as this is not useful for training the model.
    # This was not done at an earlier stage because this data is still useful in making future predictions.
...
def combining_fixture_id(df):
    '''
    This function requires the output from the function 'mod_df()'.
    Currently this df contains the features for a single team with a target fixture.
    This function combines the home and away team features into a single row per target fixture.
    This df is then complete with features for home and away teams and is therefore ready for model training.
...
def creating_ml_df(df, making_predictions=False):
    '''
    This function requires the output from the function 'average_stats_df()'.
    It will process the average stats combining the two functions: mod_df() and combining_fixture_id() to create a df ready for model training.
